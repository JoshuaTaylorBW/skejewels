...
$ThisMonth = new dateTime(date($Year."/".$Month."/01"));

$TheFirstDayOfWeek = intval($ThisMonth->format('w'));
...
echo '<table>';
	...
	//Adding Events
			...
			$query = "SELECT id, UserID, UserFirstName, UserLastName, EventName, BeginDateTime, EndDateTime, RepeatType, RepeatEndDate, Visibility FROM events
			 WHERE EXTRACT(MONTH FROM BeginDateTime) = $Month AND EXTRACT(YEAR FROM BeginDateTime) = $Year AND Userid=$currUserId AND RepeatType = 'Once' AND Visibility != 'Private' ORDER BY BeginDateTime"
			OR DIE(mysqli_error());

			$repeatQuery = "SELECT id, UserID, UserFirstName, UserLastName, EventName, BeginDateTime, EndDateTime, RepeatType, RepeatEndDate, Visibility FROM events
						 WHERE Userid=$currUserId AND RepeatType != 'Once' AND Visibility != 'Private' ORDER BY BeginDateTime"
			OR DIE(mysqli_error());
			...
			while($row = mysqli_fetch_array($result)){
				...
				if(intval($dateTime->format('i')) == 0){
					if($row['Visibility'] == 'Busy'){
						$Days[intval($dateTime->format('d'))] .= intval($dateTime->format('h')) . $dateTime->format('A') . ' Busy ' . "<br>";
					}else{
						$Days[intval($dateTime->format('d'))] .= intval($dateTime->format('h')) . $dateTime->format('A') . ' ' . $row['EventName'] . "<br>";
					}
				}else{
					if($row['Visibility'] == 'Busy'){
						$Days[intval($dateTime->format('d'))] .= intval($dateTime->format('h')) . $dateTime->format('A') . ' Busy ' . "<br>";
					}else{
						$Days[intval($dateTime->format('d'))] .= intval($dateTime->format('h')) . ':' . $dateTime->format('i') . $dateTime->format('A') . ' ' . $row['EventName'] . "<br>";
					}
				}
				if($row['Visibility'] == "Busy"){
					$DateTimes[intval($dateTime->format('d'))] .= 'Busy: ' . $dateTime->format('h').':'. $dateTime->format('i') .' '.$dateTime->format('A') . ' - ' . $endTime->format('h').':'. $endTime->format('i') .' '.$endTime->format('A') . "<br>";
				}else{
				...
				if ($repeatSplit[0] == 'Weekly') { // if the event is repeated weekly
					for ($i=0; $i < 6; $i++) { //we go through six times because there is never a month in a year with more than six rows of days
						if($Started == 0){//after we make the first day this goes to a one
							...
							if(intval($dateTime->format('i')) == 0){
								if($row['Visibility'] == 'Busy'){
									$Days[$firstDay] .= intval($dateTime->format('h')) . $dateTime->format('A') . ' Busy ' . "<br>";
								}else{
									$Days[$firstDay] .= intval($dateTime->format('h')) . $dateTime->format('A') . ' ' . $row['EventName'] . "<br>";
								}
							}else{
								if($row['Visibility'] == 'Busy'){
									$Days[$firstDay] .= intval($dateTime->format('h')) . $dateTime->format('A') . ' Busy ' . "<br>";
								}else{
									$Days[$firstDay] .= intval($dateTime->format('h')) . ':' . $dateTime->format('i') . $dateTime->format('A') . ' ' . $row['EventName'] . "<br>";
								}
							}
									if($row['Visibility'] == "Busy"){
										$DateTimes[$firstDay] .= 'Busy: ' . $dateTime->format('h').':'. $dateTime->format('i') .' '.$dateTime->format('A') . ' - ' . $endTime->format('h').':'. $endTime->format('i') .' '.$endTime->format('A') . "<br>";
									}else{
							...
							if(intval($prevWeekly + 7) <= $LastDay){//if this day is before than the last day of the month
								...
								if($Month < MonthParse($endRepeatTime->format('M'))){//if this month is before the last month
									...
									if(intval($dateTime->format('i')) == 0){
										if($row['Visibility'] == 'Busy'){
											$Days[$prevWeekly] .= intval($dateTime->format('h')) . $dateTime->format('A') . ' Busy ' . "<br>";
										}else{
											$Days[$prevWeekly] .= intval($dateTime->format('h')) . $dateTime->format('A') . ' ' . $row['EventName'] . "<br>";
										}
									}else{
										if($row['Visibility'] == 'Busy'){
											$Days[$prevWeekly] .= intval($dateTime->format('h')) . $dateTime->format('A') . ' Busy ' . "<br>";
										}else{
											$Days[$prevWeekly] .= intval($dateTime->format('h')) . ':' . $dateTime->format('i') . $dateTime->format('A') . ' ' . $row['EventName'] . "<br>";
										}
									}
									if($row['Visibility'] == "Busy"){
										$DateTimes[$prevWeekly] .= 'Busy: ' . $dateTime->format('h').':'. $dateTime->format('i') .' '.$dateTime->format('A') . ' - ' . $endTime->format('h').':'. $endTime->format('i') .' '.$endTime->format('A') . "<br>";
									}else{
									...
									//if we are working in the mONTH that the event stops repeating
				...
				if($WeekOne == 0){
					...
					for($i=$CurrentDay; $i <= $FirstSaturday; $i++){
						if($Blanks + $CurrentDay <= 4){
							echo "<td id='Event' align='center'><a class='rightTooltip' href='#'>". $Days[$i] ."<span class='classic'>" . $DateTimes[$i] . "</span></a></td>";
							$CurrentDay++;
							$DayInWeek++;
							}else{
								echo "
